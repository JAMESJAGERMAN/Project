    ...
    if meals:
        for meal in meals:
            ...
            if image_url and image_url.strip():
                text_widget.image_create(tk.END, image=get_image_from_url(image_url))
                text_widget.insert(tk.END, "\n\n")
...
def update_functionality(selection):
    if selection == "Name":
        search_button.config(command=search_recipe)
        filter_button.config(command=None)
    elif selection == "Ingredient":
        search_button.config(command=filter_recipe)
        filter_button.config(command=None)
    elif selection == "Suggestion":
        search_button.config(command=suggest_recipe_gui)
        filter_button.config(command=None)
...
app = tk.Tk()
app.title("AI-Powered Recipe")

# Title
title_label = tk.Label(app, text="AI-POWERED RECIPE", font=("Arial", 24))
title_label.grid(column=0, row=0, columnspan=3, pady=10)

# Search bar
search_entry = ttk.Entry(app, width=50)
search_entry.grid(column=0, row=1, columnspan=3, padx=10, pady=5)

# Buttons
search_button = ttk.Button(app, text="Search by Name", command=search_recipe)
search_button.grid(column=0, row=2, padx=10, pady=5)

filter_button = ttk.Button(app, text="Search by Ingredient", command=filter_recipe)
filter_button.grid(column=1, row=2, padx=10, pady=5)

suggest_button = ttk.Button(app, text="Get Suggestion", command=suggest_recipe_gui)
suggest_button.grid(column=2, row=2, padx=10, pady=5)
